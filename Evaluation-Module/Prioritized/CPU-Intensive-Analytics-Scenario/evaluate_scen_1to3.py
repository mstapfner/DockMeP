...
def main(host='5.189.144.254', port=8086):
    ...
    query = 'SELECT value FROM "container_cpu_user_seconds_total" WHERE "name" = \'graphanalytic1to3prio\';'
    ...
    query = 'SELECT value FROM "container_cpu_user_seconds_total" WHERE "name" = \'hadoopmaster1to3prio\';'
    xresult, yresult = queryCPUUtil(client, query)
    plt.plot(xresult, yresult, label = "CPU Usage of the Data-Analytic Server Benchmark", color = "blue",linewidth=7.0)
    query = 'SELECT value FROM "container_cpu_user_seconds_total" WHERE "name" = \'hadoopslave1to31prio\';'
    xresult, yresult = queryCPUUtil(client, query)
    plt.plot(xresult, yresult, label = "CPU Usage of the Data-Analytic Client 1 Benchmark", color = "green",linewidth=7.0)
    query = 'SELECT value FROM "container_cpu_user_seconds_total" WHERE "name" = \'hadoopslave1to32prio\';'
    xresult, yresult = queryCPUUtil(client, query)
    plt.plot(xresult, yresult, label = "CPU Usage of the Data-Analytic Client 2 Benchmark", color = "green",linewidth=7.0)
    query = 'SELECT value FROM "container_cpu_user_seconds_total" WHERE "name" = \'hadoopslave1to33prio\';'
    xresult, yresult = queryCPUUtil(client, query)
    ...
    query = 'SELECT * FROM "container_memory_usage_bytes" WHERE "name" = \'graphanalytic1to3prio\';'
