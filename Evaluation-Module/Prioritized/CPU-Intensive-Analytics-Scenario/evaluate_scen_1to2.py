...
def main(host='5.189.144.254', port=8086):
    ...
    query = 'SELECT value FROM "container_cpu_user_seconds_total" WHERE "name" = \'graphanalytic1to21prio\';'
    ...
    query = 'SELECT value FROM "container_cpu_user_seconds_total" WHERE "name" = \'hadoopmaster1to2prio\';'
    xresult, yresult = queryCPUUtil(client, query)
    plt.plot(xresult, yresult, label = "CPU Usage of the Data-Analytic Server Benchmark", color = "blue",linewidth=7.0)
    query = 'SELECT value FROM "container_cpu_user_seconds_total" WHERE "name" = \'hadoopslave1to21prio\';'
    xresult, yresult = queryCPUUtil(client, query)
    plt.plot(xresult, yresult, label = "CPU Usage of the Data-Analytic Client 1 Benchmark", color = "green",linewidth=7.0)
    query = 'SELECT value FROM "container_cpu_user_seconds_total" WHERE "name" = \'hadoopslave1to22prio\';'
    xresult, yresult = queryCPUUtil(client, query)
    ...
    # give a title
    plt.title('CPU Usage CPU-Intensive-Scenario 1 to 2')
    # show legend on plot
    plt.legend()
    # save as image
    plt.savefig('./gen_images/cpu_usage_1to2.png')
    ...
    query = 'SELECT * FROM "container_memory_usage_bytes" WHERE "name" = \'graphanalytic1to21prio\';'
    ...
    query = 'SELECT * FROM "container_memory_usage_bytes" WHERE "name" = \'hadoopmaster1to2prio\';'
    memXServer, memYServer = queryMemoryUsage(client, query)
    plt.plot(memXServer, memYServer, label = "Memory Usage of Data-Analytic Server Benchmark", color="blue",linewidth=7.0)
    query = 'SELECT * FROM "container_memory_usage_bytes" WHERE "name" = \'hadoopslave1to21prio\';'
    memXServer, memYServer = queryMemoryUsage(client, query)
    plt.plot(memXServer, memYServer, label = "Memory Usage of Data-Analytic Client 1 Benchmark", color="green",linewidth=7.0)
    query = 'SELECT * FROM "container_memory_usage_bytes" WHERE "name" = \'hadoopslave1to22prio\';'
    memXServer, memYServer = queryMemoryUsage(client, query)
    ...
    # give a title
    plt.title('Memory Usage CPU-Intensive-Scenario 1 to 2')
    # show legend on plot
    plt.legend()
    # save as Image
    plt.savefig('./gen_images/mem_usage_1to2.png')
