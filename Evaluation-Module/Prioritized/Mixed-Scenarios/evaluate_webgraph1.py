...
def main(host='5.189.144.254', port=8086):
    ...
    plt.figure(num='CPU Usage Web-Graph-Benchmark', figsize=(14, 10), dpi=100, facecolor='w', edgecolor='k')

    # plotting the first in-memory benchmark
    query = 'SELECT value FROM "container_cpu_user_seconds_total" WHERE "name" = \'graphanalyticsweb1prio\';'
    xresult, yresult = queryCPUUtil(client, query)
    plt.plot(xresult, yresult, label = "CPU Usage of the Graph-Analytic Benchmark", color = "red",linewidth=7.0)
    ...
    # give a title
    plt.title('CPU Usage Web-Graph Scenario 1')
    # show legend on plot
    plt.legend()
    # save as image
    plt.savefig('./gen_images/cpu_web_graph1.png')


    """Memory Usage Plot generation"""
    plt.figure(num='Memory Usage Web-Graph-Benchmark', figsize=(14, 10), dpi=100, facecolor='w', edgecolor='k')

    # plotting the Streaming Server
    query = 'SELECT * FROM "container_memory_usage_bytes" WHERE "name" = \'graphanalyticsweb1prio\';'
    memXServer, memYServer = queryMemoryUsage(client, query)
    plt.plot(memXServer, memYServer, label = "Memory Usage of the Graph-Analytic", color="red",linewidth=7.0)
    ...
    # give a title
    plt.title('Memory Usage Web-Graph Scenario 1')
    # show legend on plot
    plt.legend()
    # save as Image
    plt.savefig('./gen_images/mem_web_graph_1.png')
    plt.grid()
    ...
    plt.legend()
    # save as image
    plt.savefig('./gen_images/request_web_graph1.png')
    ...
    plt.legend()
    # save as image
    plt.savefig('./gen_images/requests_failed_web_graph1.png')
