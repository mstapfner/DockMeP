...
def main(host='5.189.144.254', port=8086):
    ...
    plt.figure(num='CPU Usage Web-Graph-Benchmark 3', figsize=(14, 10), dpi=100, facecolor='w', edgecolor='k')

    # plotting the first in-memory benchmark
    query = 'SELECT value FROM "container_cpu_user_seconds_total" WHERE "name" = \'graphanalyticsweb31prio\';'
    ...
    query = 'SELECT value FROM "container_cpu_user_seconds_total" WHERE "name" = \'graphanalyticsweb32prio\';'
    xresult, yresult = queryCPUUtil(client, query)
    plt.plot(xresult, yresult, label = "CPU Usage of the Graph-Analytic Benchmark 2", color = "blue",linewidth=7.0)
    query = 'SELECT value FROM "container_cpu_user_seconds_total" WHERE "name" = \'graphanalyticsweb33prio\';'
    xresult, yresult = queryCPUUtil(client, query)
    plt.plot(xresult, yresult, label = "CPU Usage of the Graph-Analytic Benchmark 3", color = "green",linewidth=7.0)
    ...
    plt.legend()
    # save as image
    plt.savefig('./gen_images/cpu_web_graph3.png')
    ...
    query = 'SELECT * FROM "container_memory_usage_bytes" WHERE "name" = \'graphanalyticsweb31prio\';'
    ...
    query = 'SELECT * FROM "container_memory_usage_bytes" WHERE "name" = \'graphanalyticsweb32prio\';'
    memXServer, memYServer = queryMemoryUsage(client, query)
    ...
    query = 'SELECT * FROM "container_memory_usage_bytes" WHERE "name" = \'graphanalyticsweb33prio\';'
    memXServer, memYServer = queryMemoryUsage(client, query)
    plt.plot(memXServer, memYServer, label = "Memory Usage of the Graph-Analytic Benchmark 3 while running Web-Benchmark", color="green",linewidth=7.0)
    ...
    plt.legend()
    # save as Image
    plt.savefig('./gen_images/mem_web_graph3.png')
    ...
    plt.legend()
    # save as image
    plt.savefig('./gen_images/request_web_graph3.png')
    ...
    plt.legend()
    # save as image
    plt.savefig('./gen_images/requests_failed_web_graph3.png')
