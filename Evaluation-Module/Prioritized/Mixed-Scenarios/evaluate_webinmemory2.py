...
def main(host='5.189.144.254', port=8086):
    ...
    plt.figure(num='CPU Usage Web-InMemory-Benchmark 2', figsize=(14, 10), dpi=100, facecolor='w', edgecolor='k')

    # plotting the first in-memory benchmark
    query = 'SELECT value FROM "container_cpu_user_seconds_total" WHERE "name" = \'inmemorybenchmarkweb21prio\';'
    ...
    query = 'SELECT value FROM "container_cpu_user_seconds_total" WHERE "name" = \'inmemorybenchmarkweb22prio\';'
    xresult, yresult = queryCPUUtil(client, query)
    ...
    plt.legend()
    # save as image
    plt.savefig('./gen_images/cpu_web_inmemory2.png')


    """Memory Usage Plot generation"""
    plt.figure(num='Memory Usage Web-InMemory-Benchmark 2', figsize=(14, 10), dpi=100, facecolor='w', edgecolor='k')

    # plotting the Streaming Server
    query = 'SELECT * FROM "container_memory_usage_bytes" WHERE "name" = \'inmemorybenchmarkweb21prio\';'
    ...
    query = 'SELECT * FROM "container_memory_usage_bytes" WHERE "name" = \'inmemorybenchmarkweb22prio\';'
    memXServer, memYServer = queryMemoryUsage(client, query)
    ...
    plt.legend()
    # save as Image
    plt.savefig('./gen_images/mem_web_inmemory2.png')

    """HTTP Requsts Plot Generation"""
    # New Client because of other database
    user = 'admin'
    password = 'os234guivahdf1a'
    dbname = 'inMemory2prio'
    client = InfluxDBClient(host, port, user, password, dbname)
    ...
    plt.legend()
    # save as image
    plt.savefig('./gen_images/request_web_inmemory2.png')
    ...
    plt.legend()
    # save as image
    plt.savefig('./gen_images/requests_failed_web_inmemory2.png')
